...
- Define and document a `Props` interface in frontmatter for every reusable component. The interface documentation must explain each prop's purpose and whether it is optional.
- Type component imports and helper return values
- Use two-space indentation, single-quoted TypeScript strings, trailing commas in multiline structures, and semicolons.
- Comment why a component needs non-obvious markup or client behavior; do not narrate the markup.
